export function PatchDetailSkeleton() {
  return (
    <div className="animate-pulse">
      <div className="flex h-14 items-center justify-between border-b border-exia-border/40 px-8">
        <div className="flex items-center gap-3">
          ...
          <div className="h-4 w-40 rounded bg-exia-elevated" />
        </div>
      ...
      <div className="space-y-8 p-8">
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          <div className="lg:col-span-2 rounded-xl border border-exia-border/40 bg-exia-card p-5 space-y-4">
            <div className="h-3 w-28 rounded bg-exia-elevated" />
            {Array.from({ length: 6 }).map((_, j) => (
              <div key={j} className="flex items-center justify-between py-3 border-b border-exia-border/10 last:border-0">
                <div className="h-3 w-20 rounded bg-exia-elevated" />
                <div className="h-3 w-36 rounded bg-exia-elevated" />
              </div>
            ))}
          </div>
          <div className="rounded-xl border border-exia-border/40 bg-exia-card p-5 space-y-4">
            <div className="h-3 w-24 rounded bg-exia-elevated" />
            {Array.from({ length: 3 }).map((_, j) => (
              <div key={j} className="h-10 rounded-lg bg-exia-elevated" />
            ))}
          </div>
        </div>
          ...
          <div className="rounded-xl border border-exia-border/40 bg-exia-card p-5 space-y-3">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="flex items-center gap-4 py-3 border-b border-exia-border/10 last:border-0">
                ...
                <div className="h-5 w-16 rounded-full bg-exia-elevated" />
                <div className="h-3 w-20 rounded bg-exia-elevated" />
              </div>
            ))}
          </div>
